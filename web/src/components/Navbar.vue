                ...
                <div class="hidden md:flex items-center space-x-8">
                    ...
                    <div class="relative ml-4" ref="userMenuRef">
                        <button
                            ...
                            class="flex items-center gap-2 px-4 py-2 bg-gold text-white rounded-full text-sm font-medium hover:bg-gold/90 transition-colors"
                        >
                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" />
                            </svg>
                            <span v-if="authStore.isAuthenticated">{{ authStore.user?.first_name }}</span>
                            <span v-else>Espace Client</span>
                            <!-- Chevron for authenticated users -->
                            <svg v-if="authStore.isAuthenticated" class="w-4 h-4 transition-transform" :class="{ 'rotate-180': userMenuOpen }" fill="none" stroke="currentColor" viewBox="0 0 24 24">
