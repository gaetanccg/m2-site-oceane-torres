...
export const CATEGORY_DESCRIPTIONS: CategoryDescriptions = {
    ...
    Concert: "Photographier un concert, c'est capturer l'énergie brute d'un instant : les regards des artistes, la lumière des projecteurs, la communion avec le public. Chaque image raconte une émotion unique, celle d'une scène vivante où tout s'accélère. Mon objectif est de figer ces moments pour qu'ils gardent toute leur intensité bien après la dernière note.",
...
export const UPLOAD_CONFIG = {
    chunkSize: 10,                          // Max files per chunk
    maxChunkBytes: 30 * 1024 * 1024,        // 30MB max total size per chunk
    concurrentChunks: 3,                    // Number of parallel chunk uploads
    timeout: 300000,                        // 300 seconds (5 min) timeout per request
    pollInterval: 3000,                     // Poll status every 3 seconds
    maxFileSize: 50 * 1024 * 1024,          // 50MB max per file
    maxRetries: 2,                          // Retry failed chunks up to 2 times
} as const
